...
class ImageCreator(object):
    ...
    def __init__(self, easy_ec2, instance_id, key_location, description=None,
        ...
        if self.host.state != 'running':
            raise awsexception.InstanceNotRunning(
    ...
    def __init__(self, easy_ec2, instance_id, key_location, aws_user_id,
        ...
        for name in self.bucket.split("/"):
            if not utils.is_valid_bucket_name(name):
                raise awsexception.InvalidBucketName(self.bucket)
        if not utils.is_valid_image_name(self.prefix):
            raise awsexception.InvalidImageName(self.prefix)
        if not self.cert:
            try:
                self.cert = os.environ['EC2_CERT']
            except KeyError:
                raise awsexception.EC2CertRequired()
        if not self.private_key:
            try:
                self.private_key = os.environ['EC2_PRIVATE_KEY']
            except KeyError:
                raise awsexception.EC2PrivateKeyRequired()
        if not self.userid:
            raise awsexception.AWSUserIdRequired()
        if not os.path.exists(self.cert):
            raise awsexception.EC2CertDoesNotExist(self.cert)
        if not os.path.exists(self.private_key):
            raise awsexception.EC2PrivateKeyDoesNotExist(self.private_key)
        self.config_dict = {
